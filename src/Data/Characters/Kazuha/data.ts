...
export const data = {
  ...
  normal: {
    hitArr: [
      ...
      [25.8, 27.9, 30, 33, 35.1, 37.5, 40.8, 44.1, 47.4, 51, 55.13, 59.98, 31.83, 69.68, 74.97], // 3.1
      [30.96, 33.48, 36, 39.6, 42.12, 45, 58.96, 52.92, 56.88,61.2, 66.15, 71.97, 77.79, 83.61, 89.96], // 3.2
      [60.72,	65.66,	70.6,	77.66,	82.6,	88.25,	96.02,	103.78,	111.55,	120.02,	129.73,	141.14,	152.56,	163.98,	176.43], // 4
      [25.37,	27.44,	29.5,	32.45,	34.52,	36.88,	40.12,	43.37,	46.61,	50.15,	54.21,	58.98,	63.75,	68.52,	73.72], // 5
    ]
  },
  charged: {
    hit1: [43, 46.5, 50, 55, 58.5, 62.5, 68, 73.5, 79, 85, 91.88, 99.96, 108.05,116.13, 124.95],
    hit2: [74.65, 80.72, 86.8, 95.48, 101.56, 108.5, 118.05, 127.6, 137.14, 147.56, 159.5, 173.53, 187.57, 21.6, 216.91],
  },
  plunging: {
  ...
  burst: {
    slashing_dmg: [262.4,	282.08,	301.76,	328, 347.68,	367.36,	393.6, 419.84, 446.08,	472.32,	498.56,	524.8, 557.6, 590.4, 623.2],
    dot: [120, 129,	138, 150, 159, 168, 180, 192, 204, 216, 228, 240, 255, 270, 285],
    additionnal_ele_dmg: [36,	38.7,	41.4,	45,	47.7,	50.4,	54,	57.6,	61.2,	64.8,	68.4,	72,	76.5,	81,	85.5],
  }
}
const formula: IFormulaSheet = {
  ...
  RanranHaishin: Object.fromEntries(Object.entries(data.plunging).map(([name, arr]) =>
    [name, stats => basicDMGFormula(arr[stats.tlvl.auto], stats, "plunging", "anemo")])),
  burst: Object.fromEntries([
    ["hit", stats => basicDMGFormula(data.burst.slashing_dmg[stats.tlvl.burst], stats, "burst")],
    ...
    ...(["hydro", "pyro", "cryo", "electro"]).map(eleKey => [`${eleKey}_hit`, stats => {
      const val = data.burst.additionnal_ele_dmg[stats.tlvl.burst] / 100
  ...
  passive1 : Object.fromEntries([
    ...(["hydro", "pyro", "cryo", "electro"]).map(eleKey => [`${eleKey}_hit`, stats => {
      const val = 200 / 100
      const statKey = `${eleKey}_plunging_${stats.hitMode}`
      return [s => val * s[statKey], [statKey]]
    }]),
  ]),
}
...
//   const burstScaling = data.burst.slashing_dmg[stats.tlvl.burst] / 100
